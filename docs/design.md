...
- **users** — `id`, `email citext unique not null`, `username citext unique` (FR-A1 cho phép email *hoặc* tên đăng nhập), `display_name text not null`, `role text not null check (role in ('admin','mentor','member'))`, `password_hash text`, `hash_algo text` (chỉ `'argon2id'`; bỏ nhánh firebase_scrypt), `must_change_password boolean not null default true` (FR-A2), `totp_secret text` (giữ TOTP admin tuỳ chọn của imath), `disabled boolean not null default false` (FR-A4: khoá vẫn giữ bài nộp/tiến độ), `disabled_reason text` ('admin' | 'discord_kick') + `disabled_at` (drizzle/0008 — bộ quét Discord `auth/discordSweep.ts`, v0.8), `share_solutions boolean not null default true` (FR-K opt-out, drizzle/0009), `created_at`, `last_login`, `deleted_at`. Index `(role)`.
...
- **Bảng xếp hạng contest** (FR-I5): một truy vấn SQL (§7), cache 2 s. Không bảng `contest_scores`, không job rebuild; rejudge nhất quán theo cấu trúc. **v0.8**: phần CTE `scored` tách thành builder `contest/standings.ts::scoredContestSubmissions({contestId, cutoff, since})` — standings truyền một contest và mốc cắt cụ thể; **BXH toàn ban gộp contest** (`routes/member/leaderboard.ts`, `source=practice|contest|total`) truyền `contestId=null` + `memberCutoffSql` (mỗi contest tự áp mốc đóng băng) + `since` lọc theo `end_at`. Một công thức, hai nơi dùng.
...
| Docker daemon sập/khởi động lại | ping dockerode fail → worker tự đánh dấu unhealthy (bảng `workers`), ngừng claim, retry backoff; submission dồn `pending`; UI hiện vị trí hàng đợi; monitor báo (§9). Khi hồi phục (và ở mỗi lần worker khởi động — phủ cả IE do worker chết, sửa vòng 2): FR-F8 tự lành — `healIeOnStartup()` xếp lại mọi IE còn `ie_retry` trong 24 h; **`attempt` KHÔNG về 0** (sửa 08/09/2026 khi hiện thực hoá: PK của `submission_results` là `(submission_id, attempt, position)` và INSERT của lần chấm mới là `ON CONFLICT DO NOTHING`, về 0 là kết quả từng test của lần mới bị nuốt lặng lẽ). Ở trạng thái ổn định, worker chạy `healIePeriodic()` mỗi 5 phút: xếp lại IE xong hơn 2 phút với `attempt < 3`; IE `attempt ≥ 3` không phải `stale_heartbeat` bị hạ `ie_retry=false` (nghi poison) chờ nút admin — nút đó chấm lại cả bài đã hạ. |
...
| `GET /api/member/courses/:id/leaderboard` | **FR-G6 v0.8**: xếp theo tổng điểm tích luỹ (tỉ lệ × điểm tối đa theo độ khó), rồi số bài AC, tie-break `last_gain` (§2.7); cache 5 s |
| `GET /api/member/leaderboard?scope=individual\|team&window=week\|month\|all&source=practice\|contest\|total` | BXH toàn ban (trang `/bang-xep-hang`); v0.8 gộp điểm contest qua `scoredContestSubmissions` + `memberCutoffSql` (§2.7), contest thuộc kỳ theo `end_at` |
| `GET /api/member/stats/problem?itemId\|contestProblemId` | FR-E3 v0.8: thống kê gộp của bài — lượt nộp / người / đã giải, phân bố verdict và ngôn ngữ, 8 ô thời gian của bài AC tốt nhất mỗi người, "nhanh hơn X %"; không tên người; cùng cổng `resolveAccess` với đề |
| `GET /api/member/solutions?itemId\|contestProblemId&languageId&sort` · `GET /api/member/solutions/:id?…` | FR-K: bài AC tốt nhất của người khác (danh sách không mang source, chi tiết mới có) + lời giải mẫu theo FR-D7; cổng `routes/member/solved.ts` (đã AC · không cấm vận contest · `share_solutions`); `toPeerSubmission` khai `never` cho kết quả test / log / stdout |
| `GET /api/member/discussion/problem/:problemId` (+ thread / reply / pin) | thảo luận theo bài, một cấp; cùng cổng `solved.ts` — v0.8 thêm cấm vận khi bài đang trong contest mở |
...
### Delta v0.8 — năm bổ sung theo thắc mắc của BCN + trả nợ FR-F8 (08/09/2026)

Kế hoạch và bốn quyết định sản phẩm ở `docs/plan-solution-diem-discord.md`; mỗi việc một commit trên `main`.

1. **Tự khoá khi rời Discord** (`auth/discordSweep.ts`, migration 0008): API quét server bằng Bot token mỗi `DISCORD_KICK_SWEEP_MINUTES` phút, `pg_try_advisory_lock` để blue/green không quét đôi; chỉ tài khoản do cổng guild sinh ra (cờ `discord_kick_locks_password_accounts` mở rộng); bốn lan can (Discord lỗi → bỏ lượt, danh sách rỗng → lỗi, quá nửa → huỷ, không xoá); vào lại server + đăng nhập Discord → tự mở, `disabled_reason='admin'` thì không. Chạy ở API chứ không ở worker vì `DISCORD_*` chỉ có ở `.env.api` và chỉ role API được UPDATE `users`.
2. **Điểm theo độ khó** (§2.4, §2.7): `points_*` ở settings, công thức duy nhất trong `bestSubmissions` (`maxPointsSql`), bản TypeScript `maxPointsFor` có test canh khớp; mọi bảng xếp hạng đổi sang tổng điểm trước AC. `PATCH /settings` kiểm kiểu theo mặc định, kiểm hết rồi mới ghi.
3. **BXH toàn ban gộp contest** (§5, §7): CTE của standings tách thành `scoredContestSubmissions({contestId, cutoff, since})`; BXH truyền `contestId=null` + `memberCutoffSql` (mỗi contest tự áp mốc băng) + `since` theo `end_at`; ba nguồn `practice|contest|total`, `source=practice` trả đúng số cũ.
4. **Thống kê bài** (`routes/member/stats.ts`, §5): tab Thống kê mở cho mọi vai; ngữ cảnh contest chỉ nhìn contest đó trong cửa sổ; `percentile` hạng gần nhất, 8 ô theo giới hạn bài.
5. **Lời giải chia sẻ** (`routes/member/solutions.ts` + `solved.ts`, migration 0009, §5): xem/so sánh bài AC tốt nhất của người khác, lời giải mẫu FR-D7 (trước đây `mayMemberSeeSolution` không route nào gọi), **cấm vận contest áp cho cả thảo luận** — bịt kẽ hở AC-qua-khoá-rồi-đọc-thảo-luận-lúc-thi.
6. **FR-F8 trả nợ** (§3.5): `healIeOnStartup` + `healIePeriodic`; `attempt` không về 0 vì PK `submission_results`; test grep canh `worker.ts` gọi thật.

**Nợ còn lại**: bộ abuse trên VPS đích (§11, Delta P0) và deploy thật — cả hai cần máy đích.
