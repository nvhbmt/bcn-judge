...
- **Bảng xếp hạng (Standings)** — thứ hạng member trong một contest (tính từ các bài nộp trong khung thời gian contest), trong một khoá học, hoặc toàn ban (v0.8: tổng điểm tích luỹ theo độ khó trước, rồi số bài AC; bảng toàn ban gộp được điểm contest).
...
| FR-A6 | S | **Tự khoá theo Discord** (v0.8): tài khoản do cổng Discord sinh ra bị khoá trong ≤ 10 phút sau khi rời server (hoặc mất role bắt buộc), mọi phiên bị cắt; vào lại server rồi đăng nhập Discord thì tự mở. Admin khoá tay thì Discord không mở. Cần bot token; có cờ mở rộng sang tài khoản admin cấp (mặc định tắt). Hỏi Discord không được thì không khoá ai. |
...
| FR-E3 | M | Khung nội dung (khung đầu) có tab: **Đề bài** · **Bài nộp** (lịch sử của tôi) · với mentor/admin thêm **Testcase** (xem và soạn testcase của bài đang mở — FR-D4, FR-D6) · **Thống kê** (phân bố verdict, số lượt nộp của bài đang mở). **v0.8:** *Thống kê* là một mục trên thanh icon, mở cho **mọi vai** ở cả khoá lẫn contest: số lượt nộp / người / người đã giải, phân bố verdict và ngôn ngữ, phân bố thời gian của bài AC tốt nhất mỗi người, và dòng "bạn nhanh hơn X % người đã AC"; chỉ số gộp, không tên người; ngữ cảnh contest chỉ tính bài nộp của contest đó trong cửa sổ. Bảng xếp hạng không phải tab ở đây mà mở từ thanh icon (FR-E7). |
...
| FR-F8 | M | Bài nộp bị `IE` (lỗi hệ thống) không tính vào lượt của member, được tự động chấm lại khi judge hoạt động trở lại. **v0.8 (08/09):** worker xếp lại khi khởi động và mỗi 5 phút; IE ba lần liên tiếp bởi worker đang khoẻ thì dừng tự chấm lại, nằm ở trang Tình trạng chấm chờ admin bấm. |
