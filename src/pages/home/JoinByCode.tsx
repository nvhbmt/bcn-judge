...
export function JoinByCode({ role }: { role: string }) {
  ...
  const thamGia = useMutation({
    mutationFn: (ma: string) => api.post<{ name: string }>('/api/member/courses/tham-gia', { code: ma }),
    onSuccess: (data) => {
      setLoi(null)
      setCode('')
    ...
    onError: (err) => {
      setXong(null)
      setLoi(err instanceof ApiFailure ? err.error.message : 'Không tham gia được, thử lại sau.')
    },
  })
  ...
  return (
    ...
    >
      <label htmlFor="ma-khoa" className="mb-2 block font-mono text-[12px] tracking-[0.14em] text-(--label) uppercase">
        Vào khoá bằng mã
      </label>
      <div className="flex flex-wrap items-center gap-2">
        ...
        />
        <Button type="submit" variant="primary" disabled={!code.trim() || thamGia.isPending}>
      ...
      <p className="mt-2 text-[13px] text-ink-5">
        Chỉ khoá có bật tự ghi danh mới vào được bằng mã. Khoá khác thì nhờ mentor ghi danh.
      </p>
    </form>
  )
}
