...
export function ContestListPage() {
  ...
  return (
    <PageContainer>
      <header className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-5">
            Trạng thái <i>Sắp diễn ra / Đang diễn ra / Đã kết thúc</i> suy ra từ
            khung thời gian.
          </p>
        </div>
        <Button
      ...
      {isError ? (
        <Notice tone="error">Không đọc được danh sách contest.</Notice>
      ) : null}
      ...
      {creating && !courses ? (
        <Spinner label="Đang tải danh sách khoá…" />
      ) : null}
      {creating && courses ? (
        <div className="mb-5">
          <ContestForm
            ...
            initial={{
              ...EMPTY_CONTEST,
              courseId: isAdmin ? null : (courses[0]?.id ?? null),
            }}
            courses={courses}
            canPickClub={isAdmin}
            ...
            onSubmit={(body) => {
              ...
              create.mutate(body, {
                ...
                onError: (err) =>
                  setError(readApiMessage(err, "Không tạo được contest.")),
              });
            }}
          />
        </div>
      ...
      {!isLoading && contests.length === 0 && !isError ? (
        <EmptyState
          title="Chưa có contest nào"
          hint="Tạo contest đầu tiên bằng nút phía trên."
        />
      ) : null}
        ...
        {contests.map((row) => (
          <ContestCard
            ...
            onClone={() => {
              ...
              clone.mutate(row.id, {
                onSuccess: (created) =>
                  navigate(`/mentor/contest/${created.id}`),
                onError: (err) =>
                  setError(readApiMessage(err, "Không nhân bản được contest.")),
              });
            }}
          />
        ))}
