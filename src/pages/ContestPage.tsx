...
export function ContestPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useContestDetail(contestId, { refetchInterval: 30_000 });

  const target =
    data?.phase === "sap-dien-ra" ? data.startAt : (data?.endAt ?? null);
  // Hết giờ thì tự refetch để đề mở ra mà không cần tải lại trang (FR-I3).
  const remaining = useCountdown(
    target,
    (meta?.serverTime as string) ?? null,
    () => void refetch(),
  );
  ...
  const freezeAt =
    data.freezeMinutes > 0
      ? new Date(new Date(data.endAt).getTime() - data.freezeMinutes * 60_000)
      : null;

  return (
    <div className="grid h-full lg:grid-cols-[minmax(0,1fr)_540px]">
      <main className="min-w-0 overflow-y-auto px-7 py-8">
        <p className="mb-2 flex items-center gap-2 font-mono text-[12px] tracking-[0.14em] text-ink-4 uppercase">
          <span
            aria-hidden
            className={cn("size-1.75 rounded-full", PHASE_DOT[data.phase])}
          />
        ...
        <p className="num mt-2 font-mono text-[14px] text-ink-5">
          {ngay(data.startAt)} · {hhmm(data.startAt)} → {hhmm(data.endAt)} ·{" "}
          {data.problemCount} bài · {diemToiDa} điểm tối đa
        </p>
          ...
          <div className="mt-6" aria-live="polite">
            ...
            {running && freezeAt ? (
              <p className="num mt-2 font-mono text-[12px] text-ink-5">
                BXH đóng băng lúc {hhmm(freezeAt.toISOString())}
              </p>
            ) : null}
          </div>
        ) : null}
        ...
        {data.phase === "da-ket-thuc" ? (
          <p className="mt-6 border-l-2 border-earth bg-(--tint-earth) px-3 py-2 text-[13px] text-ink-3">
            Chế độ luyện tập — bài nộp vẫn được chấm nhưng không tính vào bảng
            xếp hạng.
          </p>
        ) : null}
        ...
        <div className="mt-8">
          ...
          <div className="mt-3">
            {data.phase === "sap-dien-ra" ? (
              <EmptyState
                title="Đề chưa mở"
                hint="Danh sách bài sẽ hiện đúng giờ bắt đầu."
              />
            ) : (
              <ContestProblems
                contestId={contestId!}
