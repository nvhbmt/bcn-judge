...
export function TeamMembers({ team }: { team: TeamView }) {
  ...
  const { data: progress, isLoading } = useQuery({
    ...
    queryFn: () => api.get<TeamProgressRow[]>(`/api/member/teams/${team.id}/progress`),
    ...
    <ul className="flex flex-col gap-px bg-line">
      {team.members.map((m) => {
        ...
        const nopCuoi = cua.reduce<string | null>(
          (max, r) => (r.lastSubmittedAt && (!max || r.lastSubmittedAt > max) ? r.lastSubmittedAt : max),
          null,
        )
          ...
          <>
            ...
            {team.isLeader ? (
              <>
                ...
                <ChevronRight size={14} aria-hidden className="shrink-0 text-ink-6" />
              </>
            ) : null}
          </>
        )
          ...
          <li key={m.id} className="bg-surface-2">
            {team.isLeader ? (
              <Link
                ...
                className="flex w-full items-center gap-3 px-4 py-2.5 no-underline transition-colors duration-[120ms] ease-linear hover:bg-surface-sel focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-moss"
              >
                {than}
              </Link>
            ) : (
